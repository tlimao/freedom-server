...
class AuthRepositoryImpl(AuthRepository):
    ...
    def store_challenge(self, account_id: str, device_id: str, challenge: str) -> None:
        self._redis_connection.setex(
            f"{self.AUTH_DIRECTORY}:{account_id}:{device_id}",
            time=self.CHALLENGE_EXPIRATION_TIME,
            value=challenge)

    def get_challenge(self, account_id: str, device_id: str) -> str:
        challenge: bytes = self._redis_connection.get(f"{self.AUTH_DIRECTORY}:{account_id}:{device_id}")
    ...
    def delete_challenge(self, account_id: str, device_id: str) -> None:
        self._redis_connection.delete(f"{self.AUTH_DIRECTORY}:{account_id}:{device_id}")
